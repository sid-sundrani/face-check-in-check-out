...
for frame_no in frame_intervals:
    ...
    for name, last_seen in zip(identities, times_seen):
        if name in check_out_details:
            if datetime.now() - check_out_details[name][-1] > timedelta(seconds=2):
                check_out_details[name].append(last_seen)
        else:
            check_out_details[name] = []
            check_out_details[name].append(last_seen)
    # writing frame
...
json_file = json.dumps(check_out_details, indent=4, sort_keys=True, default=str)
f = open("check_out_times.json", "w")
f.write(json_file)
f.close()
